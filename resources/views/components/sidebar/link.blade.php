...
@php
    $isActiveClasses =  $isActive ? 'bg-primary-cream hover:bg-primary-cream-2 text-primary-green px-3' : 'text-primary-cream hover:text-primary-green hover:bg-gray-100 dark:hover:text-gray-300 dark:hover:bg-dark-eval-2';

    $classes = 'flex-shrink-0 flex items-center gap-3 p-2 transition-colors rounded-md overflow-hidden ' . $isActiveClasses;
...
@else
    <a {{ $attributes->merge(['class' => $classes]) }}>
        ...
        <span
            class="text-lg font-semibold font-sans"
            x-show="isSidebarOpen || isSidebarHovered"
        >
            {{ $title }}
        </span>
